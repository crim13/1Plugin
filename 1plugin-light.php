...
define('ONEPLUGIN_LIGHT_VERSION', '2.9.79-dev');
...
function oneplugin_light_is_divi5_module_enabled($module_key) {
    ...
    if ($module_key === 'faq') {
        $faq_extension = is_array($settings) && array_key_exists('extension_faq_enabled', $settings) ? $settings['extension_faq_enabled'] : '1';
...
$oneplugin_light_divi5_logo_server = __DIR__ . '/modules/logo/server/index.php';
if (oneplugin_light_is_divi5_module_enabled('logo') && file_exists($oneplugin_light_divi5_logo_server)) {
    require_once $oneplugin_light_divi5_logo_server;
}
...
function oneplugin_light_enqueue_divi5_logo_visual_builder_assets() {
    if (!oneplugin_light_can_enqueue_divi5_visual_builder_assets('logo')) {
        return;
    }

    $script_path = ONEPLUGIN_LIGHT_PATH . 'modules/logo/visual-builder/build/oneplugin-divi5-logo.js';
    if (!file_exists($script_path)) {
        return;
    }
    ...
    \ET\Builder\VisualBuilder\Assets\PackageBuildManager::register_package_build([
        'name'    => 'oneplugin-light-divi5-logo-visual-builder',
        'version' => $script_version,
        'script'  => [
            'src'               => ONEPLUGIN_LIGHT_URL . 'modules/logo/visual-builder/build/oneplugin-divi5-logo.js',
            'deps'              => [
                'react',
                'jquery',
                'divi-module-library',
...
add_action('divi_visual_builder_assets_before_enqueue_scripts', 'oneplugin_light_enqueue_divi5_logo_visual_builder_assets');
