...
const getSourceType = (attrs) => {
  const sourceType = getMenuSettingValue(attrs, 'sourceType', 'menu');
  return ['menu', 'location'].includes(sourceType) ? sourceType : 'menu';
};

const getMenuLocation = (attrs) => {
  return getMenuSettingValue(attrs, 'menuLocation', '');
};

const getSubmenuIndicatorIcon = (attrs) => serializeSettingValue(attrs?.submenuIndicator?.innerContent) || '▾';

const MENU_PREVIEW_SETTINGS = [
  ['sourceType', 'source_type', 'menu'],
  ...
  ['toggleBackgroundColor', 'toggle_background_color', 'transparent'],
  ...
  ['mobileOverlayColor', 'mobile_overlay_color', 'rgba(17,24,39,.5)'],
];
...
const createMetadata = ({ menus, locations }) => {
  ...
  nextMetadata.attributes.menuSettings.settings.innerContent.items.menuLocation.component.props.options = buildLocationOptions(locations);
...
const loadMenus = () => {
  ...
  MENU_CACHE.promise = window
    ...
    .then((payload) => {
      ...
      MENU_CACHE.locations = Array.isArray(payload?.data?.locations) ? payload.data.locations : [];
      return {
        menus: MENU_CACHE.menus,
        locations: MENU_CACHE.locations,
      };
    })
    .catch(() => {
      MENU_CACHE.menus = [];
      MENU_CACHE.locations = [];
      return {
        menus: MENU_CACHE.menus,
        locations: MENU_CACHE.locations,
      };
...
const getPreviewParams = (attrs) => {
  const params = {
    ...
    menu_location: getMenuLocation(attrs),
...
const PreviewFallback = ({ failed, hasSource }) => {
  if (failed) {
    return (
  ...
  if (!hasSource) {
    return (
      <div style={{ color: '#6b7280', fontSize: '14px', lineHeight: 1.5 }}>
        Select a WordPress menu or theme location from Content.
      </div>
    );
  }
...
class MenuModuleEdit extends React.Component {
  constructor(props) {
    ...
    this.setPreviewNode = (node) => {
      this.previewNode = node;
    };
    this.syncRenderedPreviewMenu = this.syncRenderedPreviewMenu.bind(this);
  }

  componentDidMount() {
    this._isMounted = true;
    this.syncPreview(this.props);
    window.addEventListener('resize', this.syncRenderedPreviewMenu);
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState.previewHtml !== this.state.previewHtml) {
      this.syncRenderedPreviewMenu();
    }
  }

  componentWillUnmount() {
    this._isMounted = false;
    window.removeEventListener('resize', this.syncRenderedPreviewMenu);
  }

  syncRenderedPreviewMenu() {
    if (!this.previewNode) {
      return;
    }

    this.previewNode.querySelectorAll('[data-oneplugin-menu]').forEach((menu) => {
      const breakpoint = parseInt(menu.getAttribute('data-mobile-breakpoint') || '980', 10) || 980;
      const isMobile = window.innerWidth <= breakpoint;
      const panel = menu.querySelector('.oneplugin-menu__panel');
      const toggle = menu.querySelector('.oneplugin-menu__toggle');
      const overlay = menu.querySelector('.oneplugin-menu__overlay');
      ...
      if (panel) {
        panel.setAttribute('aria-hidden', isMobile && !menu.classList.contains('is-open') ? 'true' : 'false');
      }

      if (overlay) {
        overlay.setAttribute('aria-hidden', menu.classList.contains('is-open') ? 'false' : 'true');
      }

      if (toggle && !toggle.__onepluginPreviewReady) {
        toggle.__onepluginPreviewReady = true;
      ...
      menu.querySelectorAll('.oneplugin-menu__submenu-toggle').forEach((button) => {
        if (button.__onepluginPreviewReady) {
          return;
        }

        button.__onepluginPreviewReady = true;
        button.addEventListener('click', (event) => {
          event.preventDefault();
          event.stopPropagation();
          const item = button.closest('.oneplugin-menu__item');
          if (!item) {
            return;
          }
  ...
  syncPreview(props) {
    ...
    const hasSource = previewParams.source_type === 'location'
      ? !!previewParams.menu_location
      : !!previewParams.menu_id;
  ...
  render() {
    ...
    const hasSource = getSourceType(attrs) === 'location' ? !!getMenuLocation(attrs) : !!getMenuId(attrs);
    ...
    return (
      ...
      >
        ...
        {previewHtml ? (
          <div ref={this.setPreviewNode} dangerouslySetInnerHTML={{ __html: previewHtml }} />
        ) : (
          <PreviewFallback failed={previewFailed} hasSource={loading || hasSource} />
        )}
      </ModuleContainer>
    );
  }
